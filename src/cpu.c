...
void cpu_adc(cpu_t* cpu, instruction_t ins) {
  ...
  switch (ins.am) {
    ...
    case AM_ABS_Y:     r = cpu->regA + cpu->memory[(uint16_t)(ins.raw[1]            + cpu->regX)] + (cpu->status_flags & SF_CARRY); break;
  ...
  r & 0x100                      ? (cpu->status_flags |= SF_OVERFLOW) : (cpu->status_flags &= ~(SF_OVERFLOW));
...
void cpu_asl(cpu_t* cpu, instruction_t ins) {
  (cpu->regA & 0b10000000)    ? (cpu->status_flags |= SF_CARRY) : (cpu->status_flags &= ~(SF_CARRY));
  switch (ins.am) {
    case AM_ACCUMULATOR: cpu->regA <<= 1;                                                            break;
    case AM_ZP:          cpu->regA <<= (cpu->memory[(uint16_t)((uint8_t)ins.raw[1])]);             break;
    case AM_ZP_X:        cpu->regA <<= (cpu->memory[(uint16_t)((uint8_t)ins.raw[1] + cpu->regX)]); break;
...
void cpu_stx(cpu_t* cpu, instruction_t ins) {
  switch (ins.am) {
    case AM_ZP:    cpu->regX = cpu->memory[(uint16_t)((uint8_t)(ins.raw[1]            ))]; break;
    case AM_ZP_Y:  cpu->regX = cpu->memory[(uint16_t)((uint8_t)(ins.raw[1] + cpu->regY))]; break;
    case AM_ABS:   cpu->regX = cpu->memory[(uint16_t)ins.raw[1]];                          break;
    default:           assert(0 && "Fatal default");
  }
}
...
#define STR_APPEND(str, fmt, ...) str += snprintf(str, 50, fmt, ##__VA_ARGS__)
  ...
  if (ins.bytes == 0 && cc == 0b00000000) {
    ...
    switch (bbb) {
      ...
      case 0b00000101: STR_APPEND(str_rep, "$%02X, X"    , cpu->memory[index + 1]);                           ins.bytes += 1; ins.am = AM_IND_X; break; // indirect, X
  ...
  // Group 2
    ...
    switch (aaa) {
      case 0b00000000: STR_APPEND(str_rep, "%s", "ASL "); ins.inst = INS_ASL; break;
      case 0b00000001: STR_APPEND(str_rep, "%s", "ROL "); ins.inst = INS_ROL; break;
      case 0b00000010: STR_APPEND(str_rep, "%s", "LSR "); ins.inst = INS_LSR; break;
      case 0b00000011: STR_APPEND(str_rep, "%s", "ROR "); ins.inst = INS_ROR; break;
      case 0b00000100: STR_APPEND(str_rep, "%s", "STX "); ins.inst = INS_STX; break;
      case 0b00000101: STR_APPEND(str_rep, "%s", "LDX "); ins.inst = INS_LDX; break;
      case 0b00000110: STR_APPEND(str_rep, "%s", "DEC "); ins.inst = INS_DEC; break;
      case 0b00000111: STR_APPEND(str_rep, "%s", "INC "); ins.inst = INS_INC; break;
